...
class P3D(nn.Module):
    ...
    def __init__(self, block, layers, modality='RGB',
        ...
        self.fc = nn.Linear(512 * block.expansion, 400) #num_classes 400 hardcoded
...
def P3D199(pretrained=False,modality='RGB',num_classes=400):
    """construct a P3D199 model based on a ResNet-152-3D model.
    """
    model = P3D(Bottleneck, [3, 8, 36, 3], modality=modality,num_classes=num_classes)
    if pretrained==True:
        if modality=='RGB':
...
if __name__ == '__main__':
    ...
    model = model.cuda()
    data=torch.rand(1,3,16,160,160).cuda()   # if modality=='Flow', please change the 2nd dimension 3==>2
    out=model(data)
    print (out.size(),out)
