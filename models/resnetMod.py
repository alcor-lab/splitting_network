...
__all__ = ['ResNet', 'resnet18', 'resnet34', 'resnet50', 'resnet101',
           'resnet152','DenseNet', 'densenet121', 'densenet169', 'densenet201', 'densenet161',
...
def resnet18(pretrained=False, noBN=False, GPU = 0, **kwargs):
...
def resnet34(pretrained=False, noBN=False, GPU = 0,**kwargs):
...
class DenseNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out2 = F.relu(features, inplace=True)
        out = F.adaptive_avg_pool2d(out2, (1, 1)).view(features.size(0), -1)
        out = self.classifier(out)
        return out, out2, out2

def densenet161(pretrained=False, noBN=False, GPU = 0,**kwargs):
    ...
    model.fc = model.classifier #binding needed to be compatible with resnet implementation
    model.to(device)
    print ("Densenet161")
    return model
...
class SENet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_class = self.logits(x_feat)
        return x_class, x_feat, x_feat 
...
def senet154(pretrained=False, noBN=False, GPU = 0, **kwargs):
    num_classes = 1000
    device = torch.device("cuda:"+ str(GPU))
    ...
    model.fc = model.last_linear #binding needed to be compatible with resnet implementation
    model.to(device)
    print ("senet154")
    return model
